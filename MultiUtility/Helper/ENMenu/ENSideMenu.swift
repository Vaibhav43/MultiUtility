...
open class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    open var menuWidth : CGFloat = 220.0 {
        didSet {
    ...
    fileprivate let clickOutsideClose = true
    fileprivate let outerView: UIView = UIView()
    ...
    func updateFrame() {
        ...
        let menuFrame = CGRect(
            ...
            y: 20,//sourceView.frame.origin.y,
            width: menuWidth,
            height: height-20
        )
    ...
    fileprivate func toggleMenu (_ shouldOpen: Bool) {
        ...
        if (bouncingEnabled) {
            ...
            if (menuPosition == .left) {
                destFrame = CGRect(x: (shouldOpen) ? -2.0 : -menuWidth, y: 20, width: menuWidth, height: height-20)
            }
            else {
