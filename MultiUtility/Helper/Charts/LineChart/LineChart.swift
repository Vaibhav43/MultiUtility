...
open class LineChart: UIView {
    ...
    public struct Dots {
        ...
        //TODO:-vaibhav
        public var innerRadius: CGFloat = 6//8
        public var outerRadius: CGFloat = 6//12
        public var innerRadiusHighlighted: CGFloat = 6//8
        public var outerRadiusHighlighted: CGFloat = 6//12
    }
    ...
    fileprivate var drawingHeight: CGFloat = 0 {
        didSet {
            ...
            var max: CGFloat = getMaximumValue()
            
            if let value = maxValue, value > max{
                max = value
            }
            
            var min: CGFloat = getMinimumValue()
            if let value = minValue, value < min{
                min = value
            }
    ...
    fileprivate var dotsLabelStore: [[UILabel]] = []
    ...
    override open func draw(_ rect: CGRect) {
        ...
        // TODO: VAIBHAV
        for label in dotsLabelStore {
            for dot in label {
                dot.removeFromSuperview()
            }
        }
        dotsLabelStore.removeAll()
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            ...
            var scalingValue:CGFloat = CGFloat(index)
            if !drawLine && index == 0{continue}
            if !drawLine && index == 1{ scalingValue = 0.5 }
            
            let xValue = self.x.scale(scalingValue) + x.axis.inset - dots.outerRadius/2
            let yValue = self.bounds.height - self.y.scale(data[index]) - y.axis.inset - dots.outerRadius/2
            //dots.outerRadius+10
            let label = UILabel()
            
            label.text = data[index].removeDecimal
            ...
            if index == 0 || index == (data.count - 1){
                diff = (index == 0) ? (-(label.intrinsicContentSize.width * 0.5)) : (label.intrinsicContentSize.width/2)
            }
            
            if index == 0, data.count == 2, data.first == minValue{
                label.textColor = .black
            }
            
            // - diff
            label.frame = CGRect(x: xValue - diff, y: yValue-20, width: label.intrinsicContentSize.width, height: 15)
        ...
        dotsLabelStore.append(labelStore)
    ...
    fileprivate func drawAxes() {
        ...
        let y0:CGFloat = height - y.axis.inset//- self.y.scale(0)
    ...
    fileprivate func getMinimumValue() -> CGFloat {
        //TODO: Vaibhav
        if var min = dataStore.first?.first{
            for data in dataStore {
                let newMin = data.min()!
                if newMin < min {
                    min = newMin
    ...
    fileprivate func drawXGrid() {
        ...
        if let data = dataStore.first, data.count > 1{
            
            var currentValue = start
            while currentValue <= stop{
                x1 = self.x.scale(currentValue) + x.axis.inset
            ...
            x1 = self.x.scale(start) + x.axis.inset
            path.move(to: CGPoint(x: x1, y: y1))
            path.addLine(to: CGPoint(x: x1, y: y2))
        }
    ...
    fileprivate func drawYGrid() {
        ...
        var currentValue = start
        while currentValue <= stop {
            y1 = self.bounds.height - self.y.scale(currentValue) - y.axis.inset
            path.move(to: CGPoint(x: x1, y: y1))
    ...
    fileprivate func drawXLabels() {
        ...
        if let xAxisData = self.dataStore.first{
            ...
            for (index, _) in xAxisData.enumerated() {
                ...
                var scalingValue: CGFloat = 1
                if !drawLine && index == 1{ scalingValue = 0.5 }
                var xValue = self.x.scale(CGFloat(index)*scalingValue) + x.axis.inset - (width / 2)
                
                //TODO: vaibhav
                if xValue+width > self.frame.width{
                    let diff = (xValue+width)-self.frame.width
                    xValue -= (diff*0.3)
                    width -= (diff/2)
                }
                else if xValue < 0{
                ...
                label.text = text
                let height = text.height(width: width, font: label.font)
                label.frame =  CGRect(x: xValue, y: y, width: width, height: height)
                self.addSubview(label)
            }
        }
    }
    ...
    fileprivate func drawYLabels() {
        ...
        while  currentValue <= stop{
            ...
            yValue = self.bounds.height - self.y.scale(currentValue) - (y.axis.inset * 1.5)
            let label = UILabel()
            label.font = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.caption2)
            label.textAlignment = .center
            self.addSubview(label)
            label.numberOfLines = 0
            label.text = currentValue.removeDecimal//rounded.removeDecimal()
            //TODO:Vaibhav
            
            let height = currentValue.removeDecimal.height(width: y.axis.inset+5, font: label.font)
            let middle = (currentValue == start) ? (height*0.5) : 5
            label.frame = CGRect(x: -5, y: yValue-middle, width: y.axis.inset+5, height: height)
        ...
        //            //TODO:Vaibhav
        //
        //            label.text = i.removeDecimal()//rounded.removeDecimal()
        //            label.frame = CGRect(x: 0, y: yValue-5, width: label.intrinsicContentSize.width, height: y.axis.inset)//y.axis.inset
        //            label.adjustsFontSizeToFitWidth = true
        //            self.addSubview(label)
        //        }
    }
